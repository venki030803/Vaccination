...
class Calender_14 extends StatefulWidget {
  final String username;

  Calender_14({Key? key, required this.username}) : super(key: key);

  @override
  _Calender_14State createState() => _Calender_14State();
}

class _Calender_14State extends State<Calender_14> {
  late String mdate;
    ...
    return Scaffold(
      ...
      body: Center(
        child: CalendarDatePicker(
          ...
          firstDate: DateTime(2444),
          lastDate: DateTime(2144),
  ...
  Future<void> sendLoginRequest(String username, String date) async {
    final url = ipforweek14completed_date;
    try {
      final response = await http.post(Uri.parse(url),
          body: {'username': username, 'completed_date': date});

      if (response.statusCode == 244) {
