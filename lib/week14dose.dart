...
import 'package:medicalprojectflutter/Calender_14.dart';
import 'package:medicalprojectflutter/ip.dart';
import 'package:medicalprojectflutter/week14qa1.dart';
...
class week14dose extends StatefulWidget {
  
  @override
  State<week14dose> createState() => _week14doseState();
}

class _week14doseState extends State<week14dose> {
  List<String> array = ["0", "0", "0", "0", "0", "0"];
   // Assuming you have userid defined somewhere

  void fetchData(String username) async {
    String apiUrl = ipforweek14doseshow;
    try {
              ...
              child: Column(
                ...
                children: [
                  ...
                  Expanded(
                    child: ListView(
                      children: [
                        buildListItem('DTwp/DTap - Dose 3', array[0]),
                        buildListItem('Hib - dose 3', array[1]),
                        buildListItem('Hep B - Dose 4', array[2]),
                        buildListItem('PCV - Dose 3', array[3]),
                        buildListItem('IPV - Dose 3', array[4]),
                        buildListItem('Rota -Dose 3', array[5]),
                      ],
                    ),
                  ),
                ],
            ...
            Center(
              child: GestureDetector(
                onTap: () {
                  Navigator.push(
                    context,
                    MaterialPageRoute(builder: (context) => week14qa1(userid)),
                  );
                },
                child: Image.asset(
  ...
  void fetchcom1(String username) async {
    String apiUrl = ipforweek14completed;
    try {
      var response = await http.post(Uri.parse(apiUrl), body: {'username': userid});
      if (response.statusCode == 200) {
  ...
  void handleResponses(String response, String userid) {
    ...
    if (values[2].substring(0, values[2].length - 1) == "0") {
      ...
      if (days > 0) {
        ...
        Navigator.push(
          context,
          MaterialPageRoute(builder: (context) => Calender_14(username: userid)),
        );
      }
    } 
    else if(values[1] == "0"){
