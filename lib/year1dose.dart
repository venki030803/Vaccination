...
import 'package:medicalprojectflutter/Calender_year_1.dart';
import 'package:medicalprojectflutter/ip.dart';
import 'calenderpopup.dart';
import 'popup1dose.dart';
import 'popup2dose.dart';

class year1dose extends StatefulWidget {


  @override
  State<year1dose> createState() => _year1doseState();
}

class _year1doseState extends State<year1dose> {
  List<int> array = [0, 0, 0, 0, 0]; // Initialize the array with integers
  final String apiUrl = ipforyear1doseshow;
  ...
  void handleResponses(String response, String username) {
    ...
    if (values[1].substring(0, values[1].length - 1) == "0") {
      ...
      if (days > 0) {
        ...
        Navigator.push(
          context,
          MaterialPageRoute(builder: (context) => Calender_year_1(username: userid)),
        );
      }
    } else {
      showDialog(
