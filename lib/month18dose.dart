...
import 'package:medicalprojectflutter/Calender_month_18.dart';
import 'package:medicalprojectflutter/ip.dart';
import 'calenderpopup.dart';
import 'popup1dose.dart';
import 'popup2dose.dart';

class month18dose extends StatefulWidget {
  @override
  State<month18dose> createState() => _month18doseState();
}

class _month18doseState extends State<month18dose> {
  List<String> array = ["0", "0", "0", "0", "0"];
  ...
  void fetchData(String username) async {
    String apiUrl = ipformonth18doseshow;
    try {
              ...
              child: Column(
                ...
                children: [
                  ...
                  Expanded(
                    child: ListView(
                      children: [
                        buildListItem('DTwP - B1/DTaP - B1', array[0]),
                        buildListItem('IPV - B1', array[1]),
                        buildListItem('Hib - B1', array[2]),
                        buildListItem('Hep - A2', array[3]),
                        
                        buildListItem('Varicella - 2', array[4]),
                      ],
                    ),
                  ),
                ],
  ...
  Widget buildListItem(String text, String isChecked, {bool showIcon = true}) {
  ...
  void fetchcom1(String username) async {
    String apiUrl = ipformonth18dose;
    try {
      var response = await http.post(Uri.parse(apiUrl), body: {'username': username});
      if (response.statusCode == 200) {
  ...
  void handleResponses(String response, String username) {
    ...
    if (values[1].substring(0, values[1].length - 1) == "0") {
      ...
      if (days > 0) {
        ...
        Navigator.push(
          context,
          MaterialPageRoute(builder: (context) => Calender_month18(username: userid)),
        );
      }
    } else {
      showDialog(
