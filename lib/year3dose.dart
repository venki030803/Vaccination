...
import 'package:medicalprojectflutter/Calender_year_3.dart';
...
class year3dose extends StatefulWidget {
  @override
  State<year3dose> createState() => _year3doseState();
}

class _year3doseState extends State<year3dose> {
  List<int> array = [0,0];
  final String apiUrl = ipforyear3doseshow;
  ...
  void handleResponses(String response, String username) {
    ...
    if (values[1].substring(0, values[1].length - 1) == "0") {
      ...
      if (days > 0) {
        ...
        Navigator.push(
          context,
          MaterialPageRoute(builder: (context) => Calender_year_3(username: userid)),
        );
      }
    } else {
      showDialog(
