...
import 'package:medicalprojectflutter/week6qa1.dart';
...
class week6dose extends StatefulWidget {
  
  @override
  State<week6dose> createState() => _week6doseState();
}

class _week6doseState extends State<week6dose> {
  List<String> array = ["0", "0", "0", "0", "0", "0"];
   // Assuming you have userid defined somewhere

  void fetchData(String username) async {
    String apiUrl = ipforweek6doseshow;
    try {
              ...
              child: Column(
                ...
                children: [
                  ...
                  Expanded(
                    child: ListView(
                      children: [
                        buildListItem('DTwp/DTap - Dose 1', array[0]),
                        buildListItem('Hib - dose 1', array[1]),
                        buildListItem('Hep B - Dose 2', array[2]),
                        buildListItem('PCV - Dose 1', array[3]),
                        buildListItem('IPV - Dose 1', array[4]),
                        buildListItem('Rota -Dose 1*', array[5]),
                      ],
                    ),
                  ),
                ],
            ...
            Center(
              child: GestureDetector(
                onTap: () {
                  Navigator.push(
                    context,
                    MaterialPageRoute(builder: (context) => week6qa1(userid)),
                  );
                },
                child: Image.asset(
  ...
  void fetchcom1(String username) async {
    String apiUrl = ipforweek6completed;
    try {
      var response = await http.post(Uri.parse(apiUrl), body: {'username': userid});
      if (response.statusCode == 200) {
  ...
  void handleResponses(String response, String userid) {
    ...
    if (values[1].substring(0, values[1].length - 1) == "0") {
      ...
      if (days > 0) {
        ...
        Navigator.push(
          context,
          MaterialPageRoute(builder: (context) => Calender(username: userid)),
        );
      }
    } else {
      print('hi2');
